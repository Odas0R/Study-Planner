...
let dropTodo = (e) => {
    ...
    if(!draggedToElement) {
        let firstDoneTodo = ul.querySelectorAll(".todo-reset")[0].closest("div");
        ul.insertBefore(draggedElement, firstDoneTodo)
        return;
    }
    ...
    if (!liOfDraggedToElement.classList.contains("todo-reset")) {
            ...
            if (ul.firstElementChild === draggedToElement) {
                ...
                if(indexOfDragged < indexOfDraggedTo) {
                    ul.insertBefore(draggedElement, draggedToElement.nextSibling);
                } else {
                    ul.insertBefore(draggedElement, draggedToElement);
                }
            }
